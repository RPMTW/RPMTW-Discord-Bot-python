...
class ExtensionManagerCog(Cog):
    ...
    ExtensionManagerSlashCommandGroup = commands.SlashCommandGroup('ext-manager', **option.default)
    ...
    @check(commands_checks.is_developer)
    async def load(self, ctx: ApplicationContext, ext_name: Option(str, choices=extension_list)):
        await self.ext_action(ctx, ext_name, 'load')
    
    @ExtensionManagerSlashCommandGroup.command(**option.default)
    @check(commands_checks.is_developer)
    async def unload(self, ctx: ApplicationContext, ext_name: Option(str, choices=extension_list)):
        await self.ext_action(ctx, ext_name, 'unload')
    
    @ExtensionManagerSlashCommandGroup.command(**option.default)
    @check(commands_checks.is_developer)
    async def reload(self, ctx: ApplicationContext, ext_name: Option(str, choices=extension_list)):
